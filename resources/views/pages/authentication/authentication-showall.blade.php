                        ...
                        <table class="table border text-nowrap text-md-nowrap table-striped table-hover">
                            <thead>
                                <tr>
                                    <th>Id</th>
                                    <th>Role</th>
                                    <th>Authentication Keys</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                {{-- @foreach ($data as $authentication)
                                    <tr>
                                        <td>{{ $authentication['id'] }}</td>
                                        <td>{{ $authentication['role_name'] }}</td>
                                        <td>{{ $authentication['authentication_key_id'] }}</td>
                                        <td>
                                            <button class="modal-effect btn btn-secondary-gradient btn-pill btn-edit"
                                                title="Edit" data-bs-effect="effect-sign" data-bs-toggle="modal"
                                                data-bs-target="#modal_" data-id="{{ $authentication['id'] }}"
                                                data-name="{{ $authentication['role_name'] }}"
                                                data-route="{{ $authentication['authentication_key_id'] }}">
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered text-center" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">Authentication</h6><button aria-label="Close" class="btn-close"
                        data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body text-start">
                    <form action="{{ route('authentication.addProcess') }}" method="POST" class="needs-validation"
                        novalidate>
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2-show-search form-select" data-placeholder="Choose Role"
                                name="role">
                            ...
                            <div class="selectgroup selectgroup-pills">
                                @foreach ($routes as $route)
                                    <label class="selectgroup-item">
                                        <input type="checkbox" name="routes[]" id="{{$route['id']}}" value="{{$route['id']}}" class="selectgroup-input">
                                        <span class="selectgroup-button">{{$route['route']}}</span>
                                    </label>
                                @endforeach
                            </div>
