    ...
    <div class="page-header">
        ...
        <div class="ms-auto pageheader-btn">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">AuthenticationKeys</li>
            </ol>
        </div>
    </div>
    <!-- PAGE-HEADER END -->
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table border text-nowrap text-md-nowrap table-striped table-hover">
                            ...
                            <tbody>
                                @foreach ($data as $authentication_key)
                                    <tr>
                                        <td>{{ $authentication_key['id'] }}</td>
                                        <td>{{ $authentication_key['name'] }}</td>
                                        <td>{{ $authentication_key['route'] }}</td>
                                        <td>{{ $authentication_key['description'] }}</td>
                                        <td><button class="modal-effect btn btn-secondary-gradient btn-pill btn-edit" title="Edit"
                                                ...
                                                data-bs-target="#modal_" data-id="{{ $authentication_key['id'] }}"
                                                data-name="{{ $authentication_key['name'] }}" data-route="{{$authentication_key['route']}}"
                                                data-description="{{ $authentication_key['description'] }}"><i
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered text-center" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">Authentication Keys</h6><button aria-label="Close" class="btn-close"
                        data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body text-start">
                    <form action="{{ route('authentication-key.addProcess') }}" method="POST" class="needs-validation" novalidate>
                        @csrf
